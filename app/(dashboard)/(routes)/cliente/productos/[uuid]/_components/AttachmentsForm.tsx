...
import { tbl_adjuntos, tbl_categorias, tbl_products } from "@prisma/client";
...
interface AttachmentFormProps {
    initialData: tbl_products & { categoria: tbl_categorias | null, adjuntos: tbl_adjuntos[] };
    id_producto: number;
};
...
export const AttachmentsForm = ({
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            await axios.post(`/api/productos/${id_producto}/attachments`, values);
            toast.success("Se agrego un elemento de referencia para el producto");
            toggleEdit();
            router.refresh();
        } catch {
    ...
    const onDelete = async (id_adjunto: number) => {
        try {
            ...
            await axios.delete(`/api/productos/${id_producto}/attachments/${id_adjunto}`);
    ...
    return (
        <div className="border bg-white shadow rounded-md p-4 mt-3">
            <div className="font-medium flex items-center justify-between">
                Adjuntos del producto
                <Button  className= "bg-[#0000ff] text-white hover:bg-black dark:hover:bg-slate-600 dark:hover:text-white" onClick={toggleEdit} variant="customghost">
                    {isEditing && (
                        <>Cancelar</>
                    )}
                    {!isEditing && (
